...
int nos_pretos_impares(NodeVP *tree)
{
    if (tree != NULL)
    {
        if ((tree->color == 1) && (tree->key % 2 != 0))
            return 1 + nos_pretos_impares(tree->left) + nos_pretos_impares(tree->right);
        else  
            return nos_pretos_impares(tree->left) + nos_pretos_impares(tree->right);
    }
    else
        return 0;
}
...
int main()
{
    ...
    printf("%d", nos_pretos_impares(tree));
